...
/*
 * This enum needs to match index in oConfigValues, otherwise we will get a
 * runtime error
 */
enum configEnum {
  aCpuThreadsConf,
...
configVal oConfigValues[] = {{aCpuThreadsConf, "cpu_threads_conf", kNullType},
...
constexpr size_t iConfigCnt =
    (sizeof(oConfigValues) / sizeof(oConfigValues[0]));
...
jconf::jconf() { prv = new opaque_private(); }
...
bool jconf::GetTlsSetting() { return prv->configValues[bTlsMode]->GetBool(); }
...
bool jconf::PreferIpv4() { return prv->configValues[bPreferIpv4]->GetBool(); }
...
bool jconf::DaemonMode() { return prv->configValues[bDaemonMode]->GetBool(); }
...
bool jconf::parse_config(const char *sFilename) {
  ...
  prv->jsonDoc.Parse<kParseCommentsFlag | kParseTrailingCommasFlag>(buffer,
                                                                    flen + 2);
  free(buffer);

  if (prv->jsonDoc.HasParseError()) {
    printer::inst()->print_msg(L0, "JSON config parse error(offset %llu): %s",
                               int_port(prv->jsonDoc.GetErrorOffset()),
                               GetParseError_En(prv->jsonDoc.GetParseError()));
    return false;
  }

  if (!prv->jsonDoc.IsObject()) { // This should never happen as we created the
                                  // root ourselves
  ...
  for (size_t i = 0; i < iConfigCnt; i++) {
    if (oConfigValues[i].iName != i) {
      printer::inst()->print_msg(L0,
                                 "Code error. oConfigValues are not in order.");
      return false;
    }

    prv->configValues[i] =
        GetObjectMember(prv->jsonDoc, oConfigValues[i].sName);

    if (prv->configValues[i] == nullptr) {
      printer::inst()->print_msg(L0,
                                 "Invalid config file. Missing value \"%s\".",
    ...
    if (!checkType(prv->configValues[i]->GetType(), oConfigValues[i].iType)) {
      printer::inst()->print_msg(
          L0, "Invalid config file. Value \"%s\" has unexpected type.",
          oConfigValues[i].sName);
      return false;
    }
  }
  ...
  for (size_t i = 0; i < GetThreadCount(); i++) {
    if (!GetThreadConfig(i, c)) {
      printer::inst()->print_msg(L0, "Thread %llu has invalid config.",
                                 int_port(i));
      return false;
    }
  }
  ...
  if (NiceHashMode() && GetThreadCount() >= 32) {
    printer::inst()->print_msg(
        L0, "You need to use less than 32 threads in NiceHash mode.");
    return false;
  }

  if (GetSlowMemSetting() == unknown_value) {
    printer::inst()->print_msg(L0, "Invalid config file. use_slow_memory must "
                                   "be \"always\", \"no_mlck\", \"warn\" or "
                                   "\"never\"");
    return false;
  }
  ...
  if (!prv->configValues[iCallTimeout]->IsUint64() ||
    ...
    printer::inst()->print_msg(L0, "Invalid config file. call_timeout, "
                                   "retry_time and giveup_limit need to be "
                                   "positive integers.");
    return false;
  }

  if (!prv->configValues[iVerboseLevel]->IsUint64() ||
      !prv->configValues[iAutohashTime]->IsUint64()) {
    printer::inst()->print_msg(L0, "Invalid config file. verbose_level and "
                                   "h_print_time need to be positive "
                                   "integers.");
    return false;
  }

  if (!prv->configValues[iHttpdPort]->IsUint() ||
      prv->configValues[iHttpdPort]->GetUint() > 0xFFFF) {
    printer::inst()->print_msg(
        L0,
  ...
  if (prv->configValues[bTlsMode]->GetBool()) {
    printer::inst()->print_msg(L0, "Invalid config file. TLS enabled while the "
                                   "application has been compiled without TLS "
                                   "support.");
    return false;
  }
#endif // CONF_NO_TLS
  ...
  if (GetSlowMemSetting() == no_mlck) {
    printer::inst()->print_msg(
        L0, "On Windows large pages need mlock. Please use another option.");
    return false;
  }
#endif // _WIN32

  printer::inst()->set_verbose_level(
      prv->configValues[iVerboseLevel]->GetUint64());
  bHaveAes = true;
  if (NeedsAutoconf())
    return true;

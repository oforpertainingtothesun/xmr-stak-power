...
inline void ROTATE_COLUMN_DOWN(uint32_t &v1, uint32_t &v2, size_t amount_bytes)
{
  uint32_t temp_var = (v1<<(8*amount_bytes))|(v2>>(8*(4-amount_bytes)));
  v2 = (v2<<(8*amount_bytes))|(v1>>(8*(4-amount_bytes)));
  v1 = temp_var;
}

inline void COLUMN(uint32_t *x, uint32_t *y, uint32_t i, uint32_t c0, uint32_t c1, uint32_t c2, uint32_t c3, uint32_t c4, uint32_t c5, uint32_t c6, uint32_t c7)
{
  uint32_t tu, tl, tv1, tv2;
  tu = T[2 * byte_from32(x, 4 * c0 + 0)];
  tl = T[2 * byte_from32(x, 4 * c0 + 0) + 1];
  tv1 = T[2 * byte_from32(x, 4 * c1 + 1)];
  tv2 = T[2 * byte_from32(x, 4 * c1 + 1) + 1];
  ROTATE_COLUMN_DOWN(tv1, tv2, 1);
  tu ^= tv1;
  tl ^= tv2;
  tv1 = T[2 * byte_from32(x, 4 * c2 + 2)];
  tv2 = T[2 * byte_from32(x, 4 * c2 + 2)+1];
  ROTATE_COLUMN_DOWN(tv1, tv2, 2);
  tu ^= tv1;
  tl ^= tv2;
  tv1 = T[2 * byte_from32(x, 4 * c3 + 3)];
  tv2 = T[2 * byte_from32(x, 4 * c3 + 3)+1];
  ROTATE_COLUMN_DOWN(tv1, tv2, 3);
  tu ^= tv1;
  tl ^= tv2;
  tl ^= T[2 * byte_from32(x, 4 * c4 + 0)];
  tu ^= T[2 * byte_from32(x, 4 * c4 + 0)+1];
  tv1 = T[2 * byte_from32(x, 4 * c5 + 1)];
  tv2 = T[2 * byte_from32(x, 4 * c5 + 1)+1];
  ROTATE_COLUMN_DOWN(tv1, tv2, 1);
  tl ^= tv1;
  tu ^= tv2;
  tv1 = T[2 * byte_from32(x, 4 * c6 + 2)];
  tv2 = T[2 * byte_from32(x, 4 * c6 + 2)+1];
  ROTATE_COLUMN_DOWN(tv1, tv2, 2);
  tl ^= tv1;
  tu ^= tv2;
  tv1 = T[2 * byte_from32(x, 4 * c7 + 3)];
  tv2 = T[2 * byte_from32(x, 4 * c7 + 3)+1];
  ROTATE_COLUMN_DOWN(tv1, tv2, 3);
  tl ^= tv1;
  tu ^= tv2;
  y[i] = tu;
...
static void RND512P(uint32_t *x, uint32_t *y, uint32_t r) {
  for(size_t i = 0 ; i < 16 ; i +=2) x[i] ^= (i * 8) ^ r;
...
static void RND512Q(uint32_t *x, uint32_t *y, uint32_t r) {
  for(size_t i = 0 ; i < 8 ; ++i) x[2 * i] = ~x[2 * i];
  for(size_t i = 0 ; i < 8 ; ++i) x[2 * i + 1] ^= uint32_t(-1 - (i << 28)) ^ r;
...
static void F512(uint32_t *h, const uint8_t *m) {
  ...
  {
    z[i] = get32(m, i);
    Ptmp[i] = h[i] ^ get32(m, i);
  }

  /* compute Q(m) */
  RND512Q(z, y, 0x00000000);
  RND512Q(y, z, 0x01000000);
  RND512Q(z, y, 0x02000000);
  RND512Q(y, z, 0x03000000);
  RND512Q(z, y, 0x04000000);
  RND512Q(y, z, 0x05000000);
  RND512Q(z, y, 0x06000000);
  RND512Q(y, z, 0x07000000);
  RND512Q(z, y, 0x08000000);
  RND512Q(y, Qtmp, 0x09000000);

  /* compute P(h+m) */
  RND512P(Ptmp, y, 0x00000000);
  RND512P(y, z, 0x00000001);
  RND512P(z, y, 0x00000002);
  RND512P(y, z, 0x00000003);
  RND512P(z, y, 0x00000004);
  RND512P(y, z, 0x00000005);
  RND512P(z, y, 0x00000006);
  RND512P(y, z, 0x00000007);
  RND512P(z, y, 0x00000008);
  RND512P(y, Ptmp, 0x00000009);
                  ...
                  BitSequence* output)
{
  ...
  for (i = SIZE512-hashbytelen; i < SIZE512; i++,j++) {
    output[j] = byte_from32(ctx->chaining, i);
  }
}
